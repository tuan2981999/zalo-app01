              ...
              <table id="order-listing" class="table">
                ...
                <tbody>
                      @foreach($xml->children() as $all_product)
                            <tr> 
                            <td><img src="{{$all_product->photo_links}}" width="70px" height="70px"/></td>  
                            <td>{{$all_product->name}}</td>
                            <td>{{$all_product->code}}</td>
                            <td>{{number_format((int)$all_product->price)}}</td>
                            <td>{{$all_product->quantity}}</td>
                            <td>{{$all_product->weight}}</td>
                      ...
                      @endforeach
                            <tr>        
                                <td colspan="6">
                                    <form class="form" action="{{URL::to('/save-file-product/'.$category_id)}}" method="post" enctype="multipart/form-data" id="add_form">
                                        {{ csrf_field() }}
